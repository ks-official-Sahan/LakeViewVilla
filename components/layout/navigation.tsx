...
import { Menu, X, Home, ImageIcon, Bed, MapPin, HelpCircle } from "lucide-react"
    ...
    <>
      <motion.header
        className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
          scrolled || isOpen ? "backdrop-blur-md bg-slate-900/90 border-b border-white/10" : "bg-transparent"
        }`}
        initial={{ y: -100 }}
        animate={{ y: 0 }}
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-200 ${
                      isActive ? "text-cyan-400 bg-white/10" : "text-white hover:text-cyan-400 hover:bg-white/5"
                    }`}
                  >
                    <Icon size={18} />
                    {item.label}
